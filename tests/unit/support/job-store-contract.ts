 ...
 */
export function describeJobStoreContract(name: string, make: () => DurableJobStore): void {
  describe(`LeasedJobStore contract — ${name}`, () => {
    ...
    it('enqueue → claimDue returns a running, leased job with attempts=1', async () => {
      const s = make();
      await s.enqueue({ workspaceId: 'ws', kind: 'k', payload: { n: 1 } });
    ...
    it('a future-scheduled job is not claimed until due', async () => {
      const s = make();
      await s.schedule({ workspaceId: 'ws', kind: 'k', payload: {} }, at(60_000));
    ...
    it('an expired lease is reclaimed and re-claimable (crash recovery)', async () => {
      const s = make();
      await s.enqueue({ workspaceId: 'ws', kind: 'k', payload: {} });
    ...
    it('fail retries until maxAttempts, then marks failed', async () => {
      const s = make();
      await s.enqueue({ workspaceId: 'ws', kind: 'k', payload: {}, maxAttempts: 2 });
