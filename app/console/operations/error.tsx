...
/**
 * Error boundary for the operations surfaces. Next.js logs the underlying error
 * (and passes only a digest in production), so this stays a clean recovery UI.
 */
export default function OperationsError({ reset }: { error: Error; reset: () => void }) {
  return (
    <div className="border-destructive/40 bg-destructive/5 mx-auto flex w-full max-w-5xl flex-col items-center rounded-2xl border border-dashed p-12 text-center">
      ...
      <h2 className="mt-4 text-sm font-semibold">Something went wrong loading operations</h2>
      <p className="text-muted-foreground mt-1 max-w-sm text-sm">
        The operations surface hit an unexpected error. Try again — if it persists, check the server
        logs.
      </p>
