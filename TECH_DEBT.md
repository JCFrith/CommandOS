...
| TD-05 | Persistence         | Operations run on a **development-only in-memory** `OperationsRepository`; there is no durable/production store yet.                                              | Add the Supabase operations adapter + migration (roadmap Sprint 3 item, deferred from this sprint per directive). |
